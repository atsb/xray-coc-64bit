...
{
	clLog( "mem usage before clear build textures surface: %llu", Memory.mem_usage() );
	...
	for(;i!=e;++i)
		::clear((*i));
	Memory.mem_compact();
	clLog( "mem usage after clear build textures surface: %llu", Memory.mem_usage() );
}
...
{	
		...
		clLog( "mem usage before mu_clear %lld", Memory.mem_usage() );
		vec_clear(_mu_models);// not clear ogf
		vec_clear(_mu_refs);
		mu_mesh_clear();
		Memory.mem_compact();
		clLog( "mem usage after mu_clear: %lld", Memory.mem_usage() );
